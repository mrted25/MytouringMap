...
class _MapScreenState extends State<MapScreen> {
  ...
  void _handleNewPosition(Position position) {
    ...
    // ========================================================
    // HITUNG HEADING DARI GPS SEBELUMNYA -> GPS SEKARANG
    // ========================================================
    ...
    if (_previousGpsLocation != null) {
      ...
      // Jangan ubah heading kalau perpindahan terlalu kecil.
      // Ini membuat marker lebih stabil ketika GPS sedikit bergeser.
      if (distance >= 2.0) {
    ...
    // Upload posisi sendiri ke Firestore secara berkala.
  ...
  void _updateDistanceAndEta() {
    ...
    final speedKmH = _isMotorMode ? 45.0 : 35.0;

    final estimatedMinutes =
        ((distanceKm / speedKmH) * 60).round();
  ...
  Future<void> _getRoute() async {
    ...
    try {
      final start =
          '${_titikKumpul.longitude},${_titikKumpul.latitude}';
  ...
  void _handleMapTap(
    ...
    if (_pickingTarget == 'start') {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Titik kumpul berhasil dipilih'),
        ),
      );
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Tujuan berhasil dipilih'),
        ),
      );
    }
  }
      ...
      builder: (dialogContext) {
        ...
        return StatefulBuilder(
          builder: (
            ...
            return AlertDialog(
              title: const Text('Create Touring'),
              content: TextField(
  ...
  Future<void> _joinFirestoreTouring({
    ...
    setState(() {
      ...
      _activeTouringCode = touringCode;
      _activeTouringName = touringName;

      _isMotorMode =
          selectedVehicle == 'motor';
    });

    _listenToMembers(touringId);
  ...
  void _listenToMembers(
    ...
    _membersSubscription = FirebaseFirestore
        .instance
        .collection('tourings')
        .doc(touringId)
        .collection('members')
  ...
  void _scheduleOwnMemberUpdate(
    ...
    if (_firestoreLocationTimer != null) {
      return;
    }
  ...
  Future<void> _updateVehicleTypeInFirestore() async {
    final touringId =
        _activeTouringId;
